...
def edge_adapt_mixed(edge_index, edge_type, topk_egde_id):
    # edge_index: [2, -1]
    # edge_type: [-1]
...
class KPCL(nn.Module):
    def __init__(self, data_config, args_config, graph, adj_mat, hp_dict=None):
        ...
        self.rp_coef = args_config.rp_coef
        self.cl_coef = args_config.cl_coef
        self.tau = args_config.cl_tau
        self.rau = args_config.ra_tau
        ...
        self.preference = BTModel(self.emb_size, tau=0.7)
    ...
    def forward(self, batch=None, ab=0):
        ...
        enc_edge_index, enc_edge_type, masked_edge_index, masked_edge_type, mask_bool = edge_adapt_mixed(edge_index, edge_type, topk_attn_edge_id)
        ...
        rp_loss = self.rp_coef * self.preference(self.intent_emb, self.gcn.relation_emb)
        ...
        cl_loss = self.ra_coef * self.contrast_fn(item_agg_ui, item_agg_kg)


        loss_dict = {
                "rec_loss": loss.item(),
                "ra_loss": rp_loss.item(),
                "cl_loss": cl_loss.item(),
            }
        return loss + rp_loss + cl_loss, loss_dict, edge_attn_score
